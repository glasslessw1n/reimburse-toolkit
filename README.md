...
> **内部使用**：本工具仅供熟悉相关业务流程的内部知情人士使用，不适用于广泛场景或通用用途。
>
> **AI 生成**：本项目由 AI 辅助生成，发布者不对使用结果承担任何责任。使用者应自行验证输出结果的准确性和合规性。
>
> **无担保**：本软件按"原样"提供，不提供任何明示或暗示的担保，包括但不限于适销性、特定用途适用性及非侵权性担保。
...
│   ├── 滴滴电子发票A.pdf
│   └── MMDD 餐饮发票 金额.pdf
├── MMDD-MMDD 目的地1 目的地2 .../           # 自驾车行程目录（命名规则相同）
│   ├── 高速路行程单 出发地-出差地.pdf
│   ├── MMDD 加油费 金额.pdf
│   ├── MMDD 通行费NN 金额.pdf              # NN=01/02/03...
...
### 自驾车出差
```
高速路行程单:  高速路行程单 出发地-出差地.pdf   例: 高速路行程单 重庆-成都.pdf
加油费:       MMDD 加油费 金额.pdf            例: 0710 加油费 350.00.pdf
通行费:       MMDD 通行费NN 金额.pdf          例: 0710 通行费01 85.50.pdf
                                              (NN 为 01/02/03 序号)
```
...
| 9 | **跨月连续性** | 跨月行程保持完整不间断 |
| 10 | **自驾车独立成段** | 自驾车行程单单独一页，加油费/通行费在其后 2×2 排列，住宿/餐饮再其后 |
